...
## 鐵則
**問題意識(problem tree)= 脊椎 + 單一事實來源。任何層不能勾稽到一個『驗證過的問題節點』,就不放行為結論。** 這擋掉所有踩過的坑(語域白地、KMeans 假群、領域汙染、低槓桿陷阱)。

## 脊椎結構
骨幹問題 → REAL_GAP/REAL 往下拆**子問題**(可行動白地在子問題層,非粗類)。每節點帶:指標+單位+部署值+物理地板+尺度+validation+確定性 headroom(物理核算)。LOW_LEVERAGE/QUALITATIVE/DRIVER 保留但**不放行為機會**。
...
## 紀律
- **永遠從脊椎出發**:換領域只換 problem tree;下游全部重算。
- 嵌入式結論先過語域扭曲檢查;分群只用局部社群非 KMeans。
- 子步驟交 [[problem-framing]](identification/decomposition/validation/anchored-harvest)。
- 資產:problem tree(架構與單一事實來源)+ 支援腳本(問題框定 / 物理核算 / 普查)。
